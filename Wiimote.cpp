  ...
  switch (controller_query_state) {
    case 0:
      ...
      if (data[1] == 0x20) {
        if (data[4] & 0x02) { // extension controller is connected
          _write_memory(connection_handle, CONTROL_REGISTER, 0xA400F0, 1, (const uint8_t[]) {
            0x55
          });
    ...
    case 1:
      ...
      if (data[1] == 0x22 && data[4] == 0x16) {
        if (data[5] == 0x00) {
          _write_memory(connection_handle, CONTROL_REGISTER, 0xA400FB, 1, (const uint8_t[]) {
            0x00
          });
    ...
    case 3:
      ...
      if (data[1] == 0x21) {
        if (memcmp(data + 5, (const uint8_t[]) {
        0x00, 0xFA
      }, 2) == 0) {
          if (memcmp(data + 7, (const uint8_t[]) {
...
static void process_l2cap_data(uint16_t connection_handle, uint16_t channel_id, uint8_t* data, uint16_t len) {
  if (data[0] == 0x03) { // CONNECTION RESPONSE
    ...
    if (wiimote_button_up) {
      _set_reporting_mode(connection_handle, 0x30, true);
    }
    if (wiimote_button_down) {
      _set_reporting_mode(connection_handle, 0x30, false);
    }
  } else {
